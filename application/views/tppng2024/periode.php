...
<center>
  ...
  <table class="table table table-condensed table-hover">
	...
	if ($this->session->userdata('level') == "ADMIN") {
		$totreal = $this->mtppng2024->gettotalreal_perperiode($v['id']);
	?>
        <td align='center'><?php echo $this->mtppng2024->get_jmlperperiode($v['id']); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalbk_perperiode($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalpk_perperiode($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalkk_perperiode($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalkp_perperiode($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($totreal,0,",","."); ?></td>
        <?php
          $totalpph = $this->mtppng2024->gettotalpph_perperiode($v['id']);
          $totaliwp = $this->mtppng2024->gettotaliwp_perperiode($v['id']);
  	  $totalbpjs = $this->mtppng2024->gettotalbpjs_perperiode($v['id']);
          $totalthp = $this->mtppng2024->gettotalthp_perperiode($v['id']);

	  $tottamb = $totalpph + $totalbpjs;
	  $totpot = $totalpph + $totaliwp + $totalbpjs;
          $bruto = $totreal + $tottamb;
	?>
	...
	?>
	  ...
	  <br/><br/>
          <?php
          echo "<form method='POST' action='../tppng2024/detailperiode'>";
	...
	$jmldata = $this->mtppng2024->get_jmlperperiode($v['id']);
	$rata_kinerja = $this->mtppng2024->getratakinerja_perperiode($v['id']);
	$rata_absensi = $this->mtppng2024->getrataabsensi_perperiode($v['id']);

	$total_bk = $this->mtppng2024->gettotalbk_perperiode($v['id']);
	$total_pk = $this->mtppng2024->gettotalpk_perperiode($v['id']);
        $total_kk = $this->mtppng2024->gettotalkk_perperiode($v['id']);
        $total_kp = $this->mtppng2024->gettotalkp_perperiode($v['id']);
