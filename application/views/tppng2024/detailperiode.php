...
<center>
  ...
  <table class='table table-condensed'>
    <tr>
      ...
      ?>	      
      <?php
	//if ($this->mtppng2024->get_statusperiode($idperiode) == "OPEN") {
	//if ($this->session->userdata('level') == "ADMIN") {
      ?>  
	...
	if ($this->mtppng2024->get_statusperiode($idperiode) == "OPEN") {
          if (($tahunperiode == '2024') AND ($bulanperiode == '15') AND ($this->mtppng2024->get_statusperiode($idperiode) == "OPEN")) { // Tambah Data THR
            ?>
                <button type="button" class="btn btn-outline btn-success" data-toggle="modal" data-target="#tampiltambahthr"><span class="fa fa-plus-square-o" aria-hidden="true"></span> Tambah Unit Kerja TPP-13 2024</button>
            <?php
          } else if (($tahunperiode == '2024') AND ($bulanperiode != '14')) {
            ?>
	...
	-->
      ...
      <td align='right' width='50'>
        <form method="POST" action="../tppng2024/periode">
  ...
  <?php
    ...
    $status = $this->mtppng2024->get_statusperiode($idperiode);
  ...
  <table class="table table table-condensed table-hover"  style='font-size: 11px;'>
      ...
      ?>
        ...
        <td align='center'><?php echo round($this->mtppng2024->getratakinerja_perbulan($v['id']),2)."<br/>".round($this->mtppng2024->getrataabsensi_perbulan($v['id']),2); ?></td>
        <td align='center'>
          <?php 
            $jnsasn = $this->mtppng2024->get_jnsasn($v['id']);
            if ($jnsasn == "PNS") {
	      ...
	      $jmlasn = $this->mkinerja_pppk->getjmlpeg_berhaktpp_perunker($v['fid_unker']);
            ...
            echo "<br/>".$this->mtppng2024->get_jmlperpengantar($v['id']);
	    echo "<br/>".$jmlasn;
          ?>
        </td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalbk_perbulan($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalpk_perbulan($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalkk_perbulan($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalkp_perbulan($v['id']),0,",","."); ?></td>
        <td align='right'><?php echo "Rp. ".number_format($this->mtppng2024->gettotalreal_perbulan($v['id']),0,",",".") ?></td>
        <?php
          $totalpph = $this->mtppng2024->gettotalpph_perbulan($v['id']);
          $totaliwp = $this->mtppng2024->gettotaliwp_perbulan($v['id']);
          $totalbpjs = $this->mtppng2024->gettotalbpjs_perbulan($v['id']);
          $totalthp = $this->mtppng2024->gettotalthp_perbulan($v['id']);

	  $totreal = $this->mtppng2024->gettotalreal_perbulan($v['id']);
	  ...
	  $thp = $totreal + $tottamb - $totpot;
        ...
        <td align='center' width='40'>
          <?php
          echo "<form method='POST' action='../tppng2024/detailperiodeunor'>";
        ...
        <?php
        $jmldata = $this->mtppng2024->get_jmlperpengantar($v['id']);
        if ($jmldata == 0) {
        ?>
        <td align='center' width='40'>
          <?php
          echo "<form method='POST' action='../tppng2024/hapuspengantar'>";
          echo "<input type='hidden' name='id_periode' id='id_periode' value='$v[fid_periode]'>";
          echo "<input type='hidden' name='id_unor' id='id_unor' value='$v[fid_unker]'>";
          echo "<input type='hidden' name='id_pengantar' id='id_pengantar' value='$v[id]'>";
          ?>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Tambah Unit Kerja TPP Periode <?php echo bulan($blnperiode); ?> 2024</h4>
      </div>
      <div class="modal-body" align="left"">   
        <form method='POST' name='formkalk' style='padding-top:8px' action='../tppng2024/tmbunorperiode_aksi'>
          ...
          <div class='row'>                                                
			...
			//if (!$this->mtppng2024->cektelahusul_pengantar($idperiode, $u['id_unit_kerja'], "PNS")) {
...
<div id="tampiltambahthr" class="modal modal-lg fade" role="dialog"  style="padding:10px;overflow:auto;width:100%;height:100%;">
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
      <!-- heading modal -->
      <div class="modal-header">
        <h4 class="modal-title">Tambah Unit Kerja TPP <?php echo bulan($blnperiode); ?> 2024</h4>
      </div>
      <div class="modal-body" align="left"">
	  <form method='POST' name='formkalk' style='padding-top:8px' action='../tppng2024/tmbunorperiode_aksi'>
          <input type='hidden' name='idperiode' id='idperiode' value='<?php echo $idperiode; ?>'>
...
<div id="updatestatus<?php echo $idperiode; ?>" class="modal fade" role="dialog">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" align="left">
        <form method='POST' name='formkalk' style='padding-top:8px' action='../tppng2024/updatestatuspengantar_aksi'>
          <input type='hidden' name='idperiode' id='idperiode' value='<?php echo $idperiode; ?>'>
          <div class='row'>
            <div class='col-md-12'>
              <div class="form-group input-group">
                  ...
                  <select class="form-control" name="idpengantar" id="idpengantar" required style="font-size: 11px;">
                    ...
                    $unor = $this->mtppng2024->get_pengantar_orderunor($idperiode)->result_array();
                    //if ($unor) {
                      foreach($unor as $u)
		      {
