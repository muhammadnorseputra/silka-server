...
{     
	...
	$this->setXY($x+255,$y+4);
        ...
        foreach ($data as $k) {
            if ($no==1) {
                ...
                $this->setXY(32,20);
                $jnsasn = $mtppng->mtppng2024->get_jnsasn($k['fid_pengantar']);
                if ($jnsasn == "PNS") {
	...
	*/
	...
	$this->lasthal = "Y";
        ...
        $this->setXY($x+55,$y+1); $this->MULTICELL(20,4,": ".$mtppng->mtppng2024->get_jmlperpengantar($k['fid_pengantar']),'','L',1);        
        $this->setXY($x+10,$y+5); $this->MULTICELL(45,4,"Rata-Rata Produktifitas Kerja",'','L',1);        
        $this->setXY($x+55,$y+5); $this->MULTICELL(20,4,": ".round($mtppng->mtppng2024->getratakinerja_perbulan($k['fid_pengantar']),2),'','L',1);
        $this->setXY($x+10,$y+9); $this->MULTICELL(45,4,"Rata-Rata Disiplin Kerja",'','L',1);
        $this->setXY($x+55,$y+9); $this->MULTICELL(20,4,": ".round($mtppng->mtppng2024->getrataabsensi_perbulan($k['fid_pengantar']),2),'','L',1);

	$totbk = round($mtppng->mtppng2024->gettotalbk_perbulan($k['fid_pengantar']));
	$totpk = round($mtppng->mtppng2024->gettotalpk_perbulan($k['fid_pengantar']));
	$totkk = round($mtppng->mtppng2024->gettotalkk_perbulan($k['fid_pengantar']));
	$totkp = round($mtppng->mtppng2024->gettotalkp_perbulan($k['fid_pengantar']));
	$tottunjbpjs = round($mtppng->mtppng2024->gettotalbpjs_perbulan($k['fid_pengantar']));
	$tottunjpph = round($mtppng->mtppng2024->gettotalpph_perbulan($k['fid_pengantar']));
	...
	//$this->setFont('arial','',9);
        ...
        //$this->setXY($x+195,$y+1); $this->MULTICELL(20,5,number_format($mtppng->mtppng2024->gettotalreal_perbulan($k['fid_pengantar']),0,",","."),'','R',1);
	...
	$totpph = round($mtppng->mtppng2024->gettotalpph_perbulan($k['fid_pengantar']));
	$totiwp = round($mtppng->mtppng2024->gettotaliwp_perbulan($k['fid_pengantar']));
	$totbpjs = round($mtppng->mtppng2024->gettotalbpjs_perbulan($k['fid_pengantar']));
	...
	$totthp = round($mtppng->mtppng2024->gettotalthp_perbulan($k['fid_pengantar']));
        $this->setFont('arial','B',9);
	...
	$this->setXY($x+5,$y+60); $this->MULTICELL(55,5,$nmver,'','C',1);
        ...
        $qrcode = $mtppng->mtppng2024->getqrcode($k['fid_pengantar']);
        $this->Image('assets/qrcodetppng2024/'.$qrcode.'.png', $x+100,$y+35,'30','30','png');

        $nipbend = $mtppng->mtppng2024->get_nipbendahara($k['fid_pengantar']);
        $nipkep = $mtppng->mtppng2024->get_nipkepala($k['fid_pengantar']);
