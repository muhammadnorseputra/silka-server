...
<center>
  ...
  <table class='table table-condensed'>
    <tr>
      ...
      ?>
        <td>
            <form class="navbar-form navbar-center" role="search" method="POST" action="../nonpns/tampildatacari">
              <div class="form-group">      
                <input type="text" name="data" id="data" class="form-control" placeholder="Ketik NIK" size='25' maxlength='25'>
                <button type="submit" class="btn btn-warning btn-sm">
                  <span class="glyphicon glyphicon-search" aria-hidden="false"></span> Cari Non PNS</button>
              </div>
            </form>  
        </td>
      <?php
      ...
      //if (($this->session->userdata('level') == "ADMIN") OR ($this->session->userdata('nonpns_priv') == "Y")) {
      ?>
      <td align='center' width='100'>
        <form method="POST" action="../nonpns/statistik">
          <input type='hidden' name='nip' id='nip' maxlength='18' value='<?php echo $v['nip']; ?>'>
          <button type="submit" class="btn btn-info btn-sm">
            <span class="glyphicon glyphicon-barcode" aria-hidden="true"></span> Statistika
          </button>
        </form>
      </td>
      <?php
