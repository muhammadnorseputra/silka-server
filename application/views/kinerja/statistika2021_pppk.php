...
<!-- Default panel contents -->
  <center>
  <div class="panel panel-warning" style="padding:5px;overflow:auto;width:100%;height:610px;">
        <div class='panel-heading' align='left'><span class="glyphicon glyphicon-file" aria-hidden="true"></span>
        <b>STATISTIK TAMBAHAN PENGHASILAN PPPK</b>
        </div>
    ...
    <div class="col-lg-2 col-md-2">
      <div class="panel panel-info">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 text-right">
                        <?php
                          $total = 0;
                          for ($i=1; $i<=12; $i++) {
                            $jml = $this->mkinerja_pppk->tottppmurni_perbulan($thn, $i);
                            if ($jml != 0) {
                              $total = $total + $jml;  
                            }                            
                          } 
        ...
        <ul class="list-group" style="padding-top:12px;overflow:auto;width:auto;height:250px;"> 
          <li class="list-group-item list-group-item-default"><center><b>JUMLAH USULAN TPP PER BULAN</b></center></li>
          <li>
          <?php
            for ($i=1; $i<=12; $i++) {
              $jml = $this->mkinerja_pppk->getjumlahusul_perperiode($thn, $i);
              if ($jml != 0) {
            ...
            <div class="panel-heading">
			  ...
			  $jmlbulan = 0;
                          for ($i=1; $i<=12; $i++) {
                            $jml = $this->mkinerja_pppk->getjumlahusul_perperiode($thn, $i);                            
                            if ($jml != 0) {
                              $kin = $this->mkinerja_pppk->getratakinerja_perbulan($thn, $i);
                              $totkin = $totkin + $kin;  
                              $jmlbulan = $i;
                            }                            
                          } 
                          ...
                          for ($i=1; $i<=12; $i++) {
                            $jml = $this->mkinerja_pppk->getjumlahusul_perperiode($thn, $i);                            
                            if ($jml != 0) {
                              $abs = $this->mkinerja_pppk->getrataabsensi_perbulan($thn, $i);
                              $totabs = $totabs + $abs;  
                              $jmlbulan = $i;
                            }                            
                          } 
          ...
          <table class="table table-bordered table-hover table-striped tablesorter">
            <thead>
              <tr>
                ...
                <th>Jumlah<br/>Usul PPPK <i class="fa fa-sort"></i></th>
            ...
            <tbody>
              <?php 
                for ($i=1; $i<=12; $i++) {
                  $jmlusul = $this->mkinerja_pppk->getjumlahusul_perperiode($thn, $i);
                  if ($jmlusul != 0) {
                    echo "<tr>";
                    echo "<td align='center'>".$i."</td>";
                    echo "<td class='danger'><p class='text-muted'>".bulan($i)." 2021</p></td>";                    
                    echo "<td>".$jmlusul."</td>";

                    $jmlwajibekin = $this->mkinerja_pppk->getjmlwajibekin($thn, $i);
                    echo "<td class='info'>".$jmlwajibekin."</td>";

                    $ratakin = $this->mkinerja_pppk->getratakinerja_perbulan($thn, $i);
                    echo "<td class='info'>".number_format($ratakin,2)."</td>";

                    $jmlekinnol = $this->mkinerja_pppk->getjmlekinnol($thn, $i);
                    echo "<td class='info'><div class='text-danger'>".$jmlekinnol."</div></td>";

                    $jmlekinmin60 = $this->mkinerja_pppk->getjmlekinmin60($thn, $i);
                    echo "<td class='info'>".$jmlekinmin60."</td>";

                    $jmlekinatas92 = $this->mkinerja_pppk->getjmlekinatas92($thn, $i);
                    echo "<td class='info'>".$jmlekinatas92."</td>";

                    $ekintertinggi = $this->mkinerja_pppk->getekintertinggi($thn, $i);
                    echo "<td class='info'>".number_format($ekintertinggi,2)."</td>";

                    $ekinterendah = $this->mkinerja_pppk->getekinterendah($thn, $i);
                    echo "<td class='info'>".number_format($ekinterendah,2)."</td>";

                    $rataabsen = $this->mkinerja_pppk->getrataabsensi_perbulan($thn, $i);
                    echo "<td class='success'>".number_format($rataabsen,2)."</td>";

                    $jmlabsennol = $this->mkinerja_pppk->getjmlabsennol($thn, $i);
                    echo "<td class='success'><div class='text-danger'>".$jmlabsennol."</div></td>";

                    $jmlabsenmin40 = $this->mkinerja_pppk->getjmlabsenmin40($thn, $i);
