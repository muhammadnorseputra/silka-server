...
{     
	...
	{                                      
                ...
                $this->cell(170,5,'Dicetak oleh '.$peg->mpegawai->getnama($login->session->userdata('nip')).' (NIP. '.$login->session->userdata('nip').') pada ' .tglwaktu_indo(date('Y-m-d H:i:s')),0,1,'R',1); //tgl_indo(date('Y-m-d') 
	...
	function Content($data)
	{
		...
		if (($idunker == '631101') OR ($idunker == '631102') OR ($idunker == '631103') OR ($idunker == '631104')OR ($idunker == '631105')
                 OR ($idunker == '631106') OR ($idunker == '631107') OR ($idunker == '631108')) {
			$nmunker = $mun->munker->getnamaunker($mpeg->mpegawai->getfidunker($k->nip));
			$this->setXY($x+60,$y+5);
	    ...
	    $jnsjab = $mkin->mkinerja->get_jnsjab($k->nip);
            ...
            //if (($jnsjab != "FUNGSIONAL TERTENTU") OR ($keltugasjft != "KESEHATAN") OR ($k->plt == "YA")) {                
	...
	$ratakinerja = $mkin->mkinerja->getratakinerja_perpengantar($k->fid_pengantar, $k->tahun, $k->bulan);
        $rataabsensi = $mkin->mkinerja->getrataabsensi_perpengantar($k->fid_pengantar, $k->tahun, $k->bulan);
        ...
        $tottppkotor = $mkin->mkinerja->tottppkotor_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
        $tottambahan = $mkin->mkinerja->tottambahan_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
        
        $tottppmurni = $mkin->mkinerja->tottppmurni_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
        $totpajak = $mkin->mkinerja->totpajak_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
	...
	//$this->setXY($x+75,$y+35);$this->MULTICELL(75,5,"",'L','L',1);
        ...
        $tottppditerimagol4 = $mkin->mkinerja->tottppditerima_perpengantarperiode_pergolru($k->fid_pengantar, $k->tahun, $k->bulan,"IV/");
        $tottppditerimagol3 = $mkin->mkinerja->tottppditerima_perpengantarperiode_pergolru($k->fid_pengantar, $k->tahun, $k->bulan,"III/");
        $tottppditerimagol2 = $mkin->mkinerja->tottppditerima_perpengantarperiode_pergolru($k->fid_pengantar, $k->tahun, $k->bulan,"II/");
        $tottppditerimagol1 = $mkin->mkinerja->tottppditerima_perpengantarperiode_pergolru($k->fid_pengantar, $k->tahun, $k->bulan,"I/");
        ...
        $tottppditerima_jpt = $mkin->mkinerja->tottppditerima_perpengantarperiode_jpt($k->fid_pengantar, $k->tahun, $k->bulan);
        $tottppditerima_administrator = $mkin->mkinerja->tottppditerima_perpengantarperiode_administrator($k->fid_pengantar, $k->tahun, $k->bulan);
        $tottppditerima_pengawas = $mkin->mkinerja->tottppditerima_perpengantarperiode_pengawas($k->fid_pengantar, $k->tahun, $k->bulan);
        $tottppditerima_jfujft = $mkin->mkinerja->tottppditerima_perpengantarperiode_jfujft($k->fid_pengantar, $k->tahun, $k->bulan);
	...
	$entrioleh = $mkin->mkinerja->getpetugasentry_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
	$entritgl = $mkin->mkinerja->gettglentry_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
	$updateoleh = $mkin->mkinerja->getpetugasupdate_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
        $updatetgl = $mkin->mkinerja->gettglupdate_perpengantarperiode($k->fid_pengantar, $k->tahun, $k->bulan);
