...
class Mtppng2024 extends CI_Model {
  ...
  function periode()
  {
    $q = $this->db->query("select * from tppng_periode where tahun='2024' order by id desc");
    return $q;
  }
  ...
  public function hitungpph($nip, $thn, $bln, $tppbruto) {
    ...
    $jnsptkp = $this->mtppng2024->get_ptkpgaji_pns($nip, $thn, $bln); // Ambil jenis PTKP dari Data Gaji
    ...
    $kategori = $this->mtppng2024->get_kategori_pp582023($jnsptkp);

    $tarif =  $this->mtppng2024->get_tarifpph21_pp582023($kategori, $pengbruto);
  ...
  public function hitungpph_pppk($nip, $thn, $bln, $tppbruto) {
    ...
    $jnsptkp = $this->mtppng2024->get_ptkpgaji_pppk($nip, $thn, $bln); // Ambil jenis PTKP dari Data Gaji
    if (($jnsptkp == '') OR ($jnsptkp == NULL)) { // jika data ptkp kosong, maka dihitung TK/0
      $jnsptkp = 'TK/0';
      $ptkp = $this->mkinerja->get_ptkp($jnsptkp);
    } else {
