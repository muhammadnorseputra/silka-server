...
public function get_usul_diklat() {
  ...
  $fetch_data = $this->md->fetch_datatable_jf($unkerid);
  $data = array();
  $no = $_POST['start'];
  foreach($fetch_data as $row) {
    ...
    $sub_array[] = "Rp. ".indorupiah($row->biaya);
    $sub_array[] = $row->capaian;
    $sub_array[] = "<textarea onBlur='update_catatan($row->id_syarat_diklat, this.value)' class='form-control' name='catatan' row='3' cols='100%' $dis>$row->ctt</textarea>";
    $sub_array[] = $status;
    $sub_array[] = '<a href="#" class="text-danger" onclick="hapus_usulan('.$row->id_syarat_diklat.')"><i class="glyphicon glyphicon-trash"></i></a>';
    $data[] = $sub_array;
  $no++;
  }
  ...
  $output = array(
    'draw'  		  => intval($_POST['draw']),
    'recordsTotal' 	  => $this->md->get_all_data_jf($unkerid),
    'recordsFiltered' => $this->md->get_filtered_data_jf($unkerid),
    'data'			  => $data			
  );
...
public function get_more_pegawai(){
  ...
  $data = $this->md->getmorepegawai('pegawai',$nipmy)->result();
  echo json_encode($data);
}
...
public function get_rekap_v2() {
  ...
  $get_data = $this->md->fetch_datatable_rekap($unker);
  $data = array();
  $no = $_POST['start'];
  ...
  $sub_array = array();
    ...
    $sub_array[] = $r->tupoksi;
    $sub_array[] = $r->jp;
    $sub_array[] = $r->penyelenggara;
    $sub_array[] = "Rp. ".indorupiah($r->biaya);
    $sub_array[] = $r->capaian;
    $sub_array[] = $status;
    $data[] = $sub_array;
  ...
  $output = array(
    'draw'  		  => intval($_POST['draw']),
    'recordsTotal' 	  => $this->md->get_all_data_rekap($unker),
    'recordsFiltered' => $this->md->get_filtered_data_rekap($unker),
    'data'			  => $data			
  );
...
public function cetaklaporan_v2($unkerid) {
  $data = $this->md->cetaklaporan_v2($unkerid)->result();
