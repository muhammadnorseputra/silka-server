...
class Workshop extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        if($params['jenis'] === 'pns') {
            $data = $this->rw->getRiwayatWorkshop($params);
        }

        if($params['jenis'] === 'pppk') {
            $data = $this->rw->getRiwayatWorkshopPppk($params);
        }
        ...
        foreach($data->result() as $row) {
            $response[] = [
                // "nip" => $row->nip,
                "no" => $row->no,
                "fid_jenis_workshop" => (int) $row->fid_jenis_workshop,
                "fid_rumpun_diklat" => (int) $row->fid_rumpun_diklat,
                ...
                "ref_jenis_workshop" => [
                    "id_jenis_workshop" => (int) $row->id_jenis_workshop,
                    "nama_jenis_workshop" => $row->nama_jenis_workshop,
                ],
                "ref_rumpun_diklat" => [
                    "id_rumpun_diklat" => (int) $row->id_rumpun_diklat,
                    "nama_rumpun_diklat" => $row->nama_rumpun_diklat,
                ]
            ];
        ...
        $result = [
            'kind' => 'Riwayat Workshop#'.$params['jenis'],
            'status' => true,
            'message' => 'Riwayat Workshop '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
