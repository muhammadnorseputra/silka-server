...
class Penghargaan extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        $data = $this->rw->getRiwayatPenghargaan($params);

        // Params Wajib
        if(empty($params['nip'])) {
            $msg = [
                'kind' => 'Riwayat Penghargaan',
                'status' => false,
        ...
        foreach($data->result() as $row) {
            $response[] = [
                "id" => $row->id,
                // "nip" => $row->nip,
                "fid_jenis_tanhor" => $row->fid_jenis_tanhor,
                "nama_tanhor" => $row->nama_tanhor,
                "tahun" => $row->tahun,
                "pejabat" => $row->pejabat,
                "no_keppres" => $row->no_keppres,
                "tgl_keppres" => $row->tgl_keppres,
                "note" => $row->note,
                ...
                "ref_jenis_tanhor" => [
                    "id_jenis_tanhor" => $row->id_jenis_tanhor,
                    "nama_jenis_tanhor" => $row->nama_jenis_tanhor,
                ]
            ];
        }
        ...
        $result = [
            'kind' => 'Riwayat Penghargaan',
            'status' => true,
            'message' => 'Riwayat Penghargaan '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
