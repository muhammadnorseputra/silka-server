...
*/
class DiklatStruktural extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        if(empty($params['jenis'])) {
            $msg = [
                'kind' => 'Riwayat Diklat#Struktural '.$params['jenis'],
                'status' => false,
        ...
        if($params['jenis'] === 'pns') {
            $data = $this->rw->getRiwayatDiklatStruktural($params);
        }
        if($params['jenis'] === 'pppk') {
            $data = $this->rw->getRiwayatDiklatStrukturalPppk($params);
        }
        ...
        foreach($data->result() as $row) {
            $response[] = [
                "id" => (int) $row->id,
                // "nip" => $row->nip,
                "fid_diklat_struktural" => $row->fid_diklat_struktural,
                ...
                "id_bkn" => $row->id_bkn,
                "ref_diklat_struktural" => [
                    "id_diklat_struktural" => (int) $row->id_diklat_struktural,
                    "nama_diklat_struktural" => $row->nama_diklat_struktural,
                ]
            ];
        ...
        $result = [
            'kind' => 'Riwayat Diklat#Struktural '.$params['jenis'],
            'status' => true,
            'message' => 'Riwayat Diklat#Struktural '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
