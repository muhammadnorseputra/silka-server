...
class Pangkat extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        $data = $this->rw->getRiwayatPangkat($params);

        // Params Wajib
        if(empty($params['nip'])) {
            $msg = [
                'kind' => 'Riwayat Pangkat',
                'status' => false,
        ...
        foreach($data->result() as $row) {
            $response[] = [
                ...
                "tmt" => $row->tmt,
                "gapok" => $row->gapok,
                "dlm_jabatan" => $row->dlm_jabatan,
                "angka_kredit" => $row->angka_kredit,
                "mkgol_thn" => $row->mkgol_thn,
                "mkgol_bln" => $row->mkgol_bln,
        ...
        $result = [
            'kind' => 'Riwayat Pangkat',
            'status' => true,
            'message' => 'Riwayat Pangkat '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
