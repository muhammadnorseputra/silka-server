...
class Anak extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        if(empty($params['jenis'])) {
            $msg = [
                'kind' => 'Riwayat Anak#'.$params['jenis'],
                'status' => false,
        ...
        if($params['jenis'] === 'pns') {
            $data = $this->rw->getRiwayatAnak($params);
        }

        if($params['jenis'] === 'pppk') {
            $data = $this->rw->getRiwayatAnakPppk($params);
        }
        ...
        foreach($data->result() as $row) {
            $response[] = [
                ...
                "ref_sutri" => [
                    ...
                    "sutri_ke" => $row->sutri_ke,
                    "tgl_nikah" => $row->tgl_nikah,
                    "no_akta_nikah" => $row->no_akta_nikah,
                    "fid_tingkat_pendidikan_hapus" => $row->fid_tingkat_pendidikan_hapus,
        ...
        $result = [
            'kind' => 'Riwayat Anak#'.$params['jenis'],
            'status' => true,
            'message' => 'Riwayat Anak '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
