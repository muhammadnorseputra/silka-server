...
class Cuti extends REST_Controller  {
    ...
    function __construct() {
        ...
        parent::__construct();
        //load model services
        $this->load->model(['api/v2/Riwayat' => 'cuti']);
        $this->load->helper(['fungsipegawai', 'api']);
        // Cek Token JWT
    ...
    public function index_get($nip=null)
    {
        ...
        $data = $this->cuti->getRiwayatCuti($params);
        ...
        foreach($data->result() as $row) {
            $response[] = [
                "id" => (int) $row->id,
                "fid_jns_cuti" => (int) $row->fid_jns_cuti,
                "thn_cuti" => (int) $row->thn_cuti,
                "jml" => (int) $row->jml,
                "satuan_jml" => $row->satuan_jml,
                "tambah_hari_tunda" => (int) $row->tambah_hari_tunda,
                ...
                "berkas" => $row->berkas,
                "ref_jenis_cuti" => [
                    "id_jenis_cuti" => (int) $row->id_jenis_cuti,
        ...
        $result = [
            'kind' => 'Riwayat Cuti',
            'status' => true,
            'message' => 'Riwayat Cuti '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
