...
*/
class DiklatFungsional extends REST_Controller  {
    ...
    public function index_get($nip=null)
    {
        ...
        if(empty($params['jenis'])) {
            $msg = [
                'kind' => 'Riwayat Diklat#Fungsional '.$params['jenis'],
                'status' => false,
        ...
        if($params['jenis'] === 'pns') {
            $data = $this->rw->getRiwayatDiklatFungsional($params);
        }
        if($params['jenis'] === 'pppk') {
            $data = $this->rw->getRiwayatDiklatFungsionalPppk($params);
        }
        ...
        foreach($data->result() as $row) {
            $response[] = [
                // "nip" => $row->nip,
                "no" => $row->no,
                "nama_diklat_fungsional" => $row->nama_diklat_fungsional,
                "tahun" => $row->tahun,
        ...
        $result = [
            'kind' => 'Riwayat Diklat#Fungsional '.$params['jenis'],
            'status' => true,
            'message' => 'Riwayat Diklat#Fungsional '.$params['nip'].' Ditemukan',
            'count' => $data->num_rows(),
