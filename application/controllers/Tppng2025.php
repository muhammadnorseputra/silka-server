...
class Tppng2025 extends CI_Controller {
  ...
  function periode() {
    $data['content'] = 'tppng2025/periode';
    $data['periode'] = $this->mtppng2025->periode()->result_array();
    $data['pesan'] = '';
    $data['jnspesan'] = '';
  ...
  function detailperiode() {
    ...
    $data['data'] = $this->mtppng2025->get_pengantar($idperiode)->result_array();
    $data['content'] = 'tppng2025/detailperiode';
    $data['pesan'] = '';
    $data['jnspesan'] = '';
    $this->load->view('template', $data);
  }
  ...
  function detailperiodeunor() {
    ...
    $data['idperiode'] = $idperiode;
    $data['data'] = $this->mtppng2025->tampil_tppng($idpengantar, $idperiode)->result_array();
    $data['content'] = 'tppng2025/detailperiodeunor';
    $data['pesan'] = '';
    $data['jnspesan'] = '';
    $this->load->view('template', $data);
  }
  ...
  function kirim_tandaterima() {
    ...
    $config['upload_path'] = './filetppng2025/'; //Folder untuk menyimpan hasil upload
    ...
    if($_FILES['berkastppng_pengantar']['name'])
        {            
          if ($this->upload->do_upload('berkastppng_pengantar'))
          {
              ...
              $this->mtppng2025->update_pembayaran($where, $datatodb);
              $this->mtppng2025->update_status('tppng', ['fid_status' => 6], ['fid_pengantar' => $idpengantar]);

              if (file_exists('./filetppng2025/'.$nmberkaslama.'.pdf')) {
                unlink('./filetppng2025/'.$nmberkaslama.'.pdf');
              } elseif (file_exists('./filetppng2025/'.$nmberkaslama.'.PDF')) {
                      unlink('./filetppng2025/'.$nmberkaslama.'.PDF');
              }	
  ...
  function tampilkalkulasi() {
    ...
    if ($jnsasn == "PNS") {
      ...
      $id_unker_nip = $this->mtppng2025->get_idunker_pns($nip);
    } else if ($jnsasn == "PPPK") {
      $nama = $this->mpppk->getnama($nip);
      $id_unker_nip = $this->mtppng2025->get_idunker_pppk($nip);
    }

    $telahusul = $this->mtppng2025->cektelahusul($nip, $thnperiode, $blnperiode);
    ...
    if (!$nama) {
	  ...
	  $jnsptkp = $this->mtppng2025->get_ptkpgaji_pns($nip, $thnperiode, $blnperiode); // Ambil jenis PTKP dari Data Gaji	
	  ...
	  $jnsptkp = $this->mtppng2025->get_ptkpgaji_pppk($nip, $thnperiode, $blnperiode); // Ambil jenis PTKP dari Data Gaji
        }
	...
	if ($pengurang == '' OR $pengurang == 'mkd7h') { // Hanya tnpa potongan dan masuk kerja 7 hari yg dicek kinerja bulanannya
          $berkas = $this->mtppng2025->get_berkasskpbulanan($nip, $thnperiode, $blnperiode);
	  if ($jnsasn == "PNS"){
	...
	if (($wajiblhkpn == "YA") AND ($blnperiode >= 4)) {
	  ...
	  $cekberkaslhkpn = $this->mtppng2025->get_berkaslhkpn($nip, '2023');
	...
	echo "</div>"; // End Kolom
	  ...
	  echo "</div>"; // End Row Penilaian dan Kalkulasi	
          ...
          if ($jnsasn == "PNS") {
            ...
            $bpjs_gaji = $this->mtppng2025->get_bpjsgaji($nip, $thnperiode, $blnperiode);
          } else if ($jnsasn == "PPPK") {
            ...
            $bpjs_gaji = $this->mtppng2025->get_bpjsgaji_pppk($nip, $thnperiode, $blnperiode);
          }

          $hasil_bruto = $gaji_bruto + $rea_bruto;

	  if ($jnsasn == "PNS") {
		$pph_bayar = $this->mtppng2025->hitungpph($nip, $thnperiode, $blnperiode, $rea_bruto);
		$ptkpgaji = $this->mtppng2025->get_ptkpgaji_pns($nip, $thnperiode, $blnperiode);
          } else if ($jnsasn == "PPPK") {
		$pph_bayar = $this->mtppng2025->hitungpph_pppk($nip, $thnperiode, $blnperiode, $rea_bruto);
		$ptkpgaji = $this->mtppng2025->get_ptkpgaji_pppk($nip, $thnperiode, $blnperiode);
          }

	  $katpajak = $this->mtppng2025->get_kategori_pp582023($jnsptkp);          
...
/*<blockquote style='font-size: 11px;'>*/
  ...
  function tmbkalk_aksi() {
    ...
    //$idgolru = $this->mpetajab->getidgolru($nip);
    $id_unker = $this->mtppng2025->get_idunker($idpengantar);
    ...
    */
    ...
    if ($jnsasn == "PNS") {
	$ptkp = $this->mtppng2025->get_ptkpgaji_pns($nip, $thnperiode, $blnperiode);
	$pph_bayar = $this->mtppng2025->hitungpph($nip, $thnperiode, $blnperiode, $rea_bruto);
    } else if ($jnsasn == "PPPK") {
	$ptkp = $this->mtppng2025->get_ptkpgaji_pppk($nip, $thnperiode, $blnperiode);
       	$pph_bayar = $this->mtppng2025->hitungpph_pppk($nip, $thnperiode, $blnperiode, $rea_bruto);
    }
    ...
    if ($this->mtppng2025->cektelahusul($nip, $thnperiode, $blnperiode) == 0) {
      if ($this->mtppng2025->input_tppng($data)) {
  ...
  function hapustppng(){    
    ...
    $id_unker = $this->mtppng2025->get_idunker($id_pengantar);
    $bln = $this->mtppng2025->get_bulanperiode($id_periode);
    $thn = $this->mtppng2025->get_tahunperiode($id_periode);
    ...
    $nama = $this->mpegawai->getnama($nip);
    if ($this->mtppng2025->hapustppng($where)) {// hapus seluruh usulan pada tabel usul_tpp
    ...
    $data['data'] = $this->mtppng2025->tampil_tppng($id_pengantar, $id_periode)->result_array();
    $data['content'] = 'tppng2025/detailperiodeunor';
    $this->load->view('template', $data);
  }
  ...
  function tmbunorperiode_aksi() {
    ...
    // START QR CODE
            ...
            $config['imagedir']     = './assets/qrcodetppng2025/'; //direktori penyimpanan qr code
    ...
    if ($this->session->userdata('level') == "ADMIN") { // ADMIN dapat menambah banyak pengantar
      ...
      $telahusul = $this->mtppng2025->cektelahusul_pengantar($idperiode, $idunor, $jnsasn);
      //$cekstatusberkasbulanlalu = $this->mtppng2025->cekstatusberkaspengantar_bulanlalu($idperiode, $idunor, $jnsasn);
      $cekstatusberkasbulanlalu = TRUE;	
    }

    if (($telahusul == 0) AND ($cekstatusberkasbulanlalu)) {
      if ($this->mtppng2025->input_tppng_pengantar($data)) {
  ...
  function hapuspengantar(){    
    ...
    if ($this->mtppng2025->hapustppng_pengantar($where)) {// hapus seluruh usulan pada tabel usul_tpp
      $qrcode = $this->mtppng2025->getqrcode($id_pengantar);
      
      if (is_file("../assets/qrcodetppng2025/".$qrcode.".png")) {
        unlink("../assets/qrcodetppng2025/".$qrcode.".png");
      }
    ...
    $data['idperiode'] = $id_periode;
    $data['data'] = $this->mtppng2025->get_pengantar($id_periode)->result_array();
    $data['content'] = 'tppng2025/detailperiode';
    $this->load->view('template', $data);
  }
  ...
  function valid_tppng() {
    ...
    $this->mtppng2025->edit_tppng($where, $data);
  ...
  function valid_pengantar() { // Kirim pengantar ke BKPSDM
    ...
    if (($this->mtppng2025->edit_tppng($where_tppng, $data_tppng)) AND ($this->mtppng2025->edit_tppngpengantar($where_pengantar, $data_pengantar))) {
  ...
  public function cetak_tandaterima()  
  {
    ...
    if ($this->mtppng2025->edit_tppngpengantar($where_pengantar, $data_pengantar)) {
      // Update status masing2 usulan PNS menjadi PRINT	
      $this->mtppng2025->edit_tppng($where_tppng, $data_tppng);
    ...
    $data['$idpengantar'] = $idpengantar;
    $data['data'] = $this->mtppng2025->cetak_tandaterima($idpengantar, $idperiode)->result_array();
    $this->load->view('/tppng2025/cetaktandaterima',$data);     
  }
  ...
  function tampilkalkulasithr() {
    ...
    $idjabpeta_tppfebruari = $this->mtppng2025->get_idjabpeta_tpp_bulanan($nip, $thnperiode, '2');
    $idjabpetaplt_tppfebruari = $this->mtppng2025->get_idjabpetaplt_tpp_bulanan($nip, $thnperiode, '2');
    ...
    if (!$nama) {
      ...
      echo "<div align='center' class='text text-warning'>JABATAN YANG DIPILIH TIDAK SESUAI DENGAN JABATAN PADA TPP PERIODE FEBRUARI 2024</div>";	
        ...
        if ($jnsasn == "PNS") {
          ...
          $tmtjab = $this->mpegawai->gettmtjabterakhir($nip);
          $npwp = $this->mkinerja->get_npwp($nip);
          $jnsptkp = $this->mtppng2025->get_ptkpgaji_pns($nip, $thnperiode, $blnperiode_gaji); // Ambil jenis PTKP dari Data Gaji
          //$jnsptkp = $this->mtppng2025->get_jnsptkp_rwykel($nip); // Ambil jenis PTKP dari Riwayat Keluarga
        } else if ($jnsasn == "PPPK") {
          $statpeg = "PPPK";
          $idgolru = $this->mpppk->getidgolruterakhir($nip);
	...
	$tppmaret = $this->mtppng2025->get_tpp_bulanan($nip, $thnperiode, '2')->result_array();
        if ($tppmaret) {
          echo "<div class='col-md-6'>
  ...
  function tmbkalkthr_aksi() {
    ...
    if ($jnsasn == "PNS") {
      ...
      $tmtjab = "";
      $npwp = $this->mkinerja_pppk->get_npwp($nip);
      $jnsptkp = $this->mtppng2025->get_ptkpgaji_pppk($nip, $thnperiode, $blnperiode_gaji); // Ambil jenis PTKP dari Data Gaji
      //$jnsptkp = $this->mkinerja_pppk->get_jnsptkp($nip);
    }
    ...
    if ($jnsasn == "PNS") {
      ...
      //$kin = $this->mkinerja->get_realisasikinerja2024($nip, $thnperiode, $blnperiode);
      $gaji = $this->mkinerja->get_gajibruto($nip, $thnperiode, $blnperiode_gaji);
      $bpjs_gaji = $this->mtppng2025->get_bpjsgaji($nip, $thnperiode, $blnperiode_gaji);
    } if ($jnsasn == "PPPK") {
      $haktpp = $this->mkinerja_pppk->get_haktpp_pppk($nip);;
      //$abs = $this->mkinerja_pppk->get_realisasiabsensi($nip, $thnperiode, $blnperiode);
      //$kin = $this->mkinerja_pppk->get_realisasikinerja2024($nip, $thnperiode, $blnperiode);
      $gaji = $this->mkinerja_pppk->get_gajibruto($nip, $thnperiode, $blnperiode_gaji);
      $bpjs_gaji = $this->mtppng2025->get_bpjsgaji_pppk($nip, $thnperiode, $blnperiode_gaji);
    }
    ...
    //$pph_bayar = $this->hitungpph($nip, $thnperiode, $blnperiode, $rea_bruto);
    if ($jnsasn == "PNS") {
        $ptkp = $this->mtppng2025->get_ptkpgaji_pns($nip, $thnperiode, $blnperiode_gaji);
        $pph_bayar = $this->mtppng2025->hitungpph($nip, $thnperiode, $blnperiode, $rea_bruto);
    } else if ($jnsasn == "PPPK") {
        $ptkp = $this->mtppng2025->get_ptkpgaji_pppk($nip, $thnperiode, $blnperiode_gaji);
        $pph_bayar = $this->mtppng2025->hitungpph_pppk($nip, $thnperiode, $blnperiode, $rea_bruto);
    }
  ...
  public function cetak_tandaterimathr()
  {
    ...
    if ($this->mtppng2025->edit_tppngpengantar($where, $data)) {
      $data['pesan'] = '<b>SUKSES</b>, Spesimen Berhasil Disimpan';
      $data['jnspesan'] = 'alert alert-success';
    } else {
      $data['pesan'] = '<b>GAGAL</b>, Spesimen Gagal Disimpan';
    ...
    $data['$idpengantar'] = $idpengantar;
    $data['data'] = $this->mtppng2025->cetak_tandaterima($idpengantar, $idperiode)->result_array();
    $this->load->view('/tppng2025/cetaktandaterimathr',$data);
  }	
  ...
  function tampilskpbulanan() {
    if ($this->session->userdata('nominatif_priv') == "Y") {
      ...
      $data['pesan'] = '';
      $data['jnspesan'] = '';
      $data['content'] = 'tppng2025/tampilskpbulanan';
      $this->load->view('template', $data);
    }
  }
  ...
  function showskpbulanan() {
    ...
    foreach($data as $dp) :
      ...
      if ($berhaktpp == 'YA') {
        $datakin = $this->mtppng2025->get_kinerja_bulanan($nip, $thn, $bln)->result_array();
  ...
  function updatestatusperorangan_aksi() {
    ...
    if ($this->mtppng2025->edit_tppng($where, $data)) {
  ...
  function tampilJabBlnLalu() {
    ...
    $tahun = $this->mtppng2025->get_tahunperiode($idperiode-1);
    $bulan = $this->mtppng2025->get_bulanperiode($idperiode-1);	
    $data = $this->mtppng2025->get_tpp_bulanan($nip, $tahun, $bulan)->result_array();
    foreach($data as $d) {
...
".$unor."</textarea>";
        echo "<input class='form-control' name='idjabpeta' id='idjabpeta' type='text' value='".$idjabpeta."'  style='font-size: 12px;'>";
        echo "</div>";
      } else {
        echo "<div class='form-group input-group'>
